...
|---|---|---|
| cooperative concurrency|   Preemptive concurrency | true parallelism|

Model Cooperative concurrencyPreemptive concurrencyTrue parallelism
Execution Single thread, event loop, Multiple threads, one processMultiple processes 
Parallelism ❌ No❌ No (GIL)✅ Yes
Best forI/O-bound (network, disk)I/O-bound (blocking libs)CPU-bound tasks
MemoryShared (low overhead)Shared (low overhead)Separate (high overhead)
SwitchingYou yield control (await)OS decidesOS decides
